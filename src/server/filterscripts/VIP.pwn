...
CMD:vinvisible(playerid)
{
	if(PlayerInfo[playerid][vip] >= 1)
	{
	    if(PlayerInfo[playerid][isInvisible])
	    {
			...
			SendClientMessage(playerid, COLOR_ORANGE, "Zviditelnil si sa na mape pred ostatnýma hráèmi!");
		}else{
			foreach(new i : Player)
			{
			...
			SendClientMessage(playerid, COLOR_ORANGE, "Zneviditelnil si sa na mape pred ostatnýma hráèmi!");
		}
	}
	return 1;
}
...
CMD:vvr(playerid)
{
	if(PlayerInfo[playerid][vip] >= 1)
	{
 		if(IsPlayerInAnyVehicle(playerid))
 		{
	    	...
	    	SendClientMessage(playerid, -1, "Vozidlo bolo opravené!");
...
CMD:vnitro(playerid)
{
	if(PlayerInfo[playerid][vip] >= 1)
	{
 		if(IsPlayerInAnyVehicle(playerid))
 		{
	    	...
	    	SendClientMessage(playerid, -1, "Do vozidla bolo pridané Nitro!");
	    	PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
