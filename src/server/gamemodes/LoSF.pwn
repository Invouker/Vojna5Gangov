...
//potom d·t do gamemodeinit + zpr·vu v kicku
...
public CheckVIP()
{
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        if(PlayerInfo[i][CasVIP] <= gettime())
        {
            ...
            Message(i, "Tvoje VIP pr·ve vypröalo !");
        }
    }
    return 1;
}
...
{
    SCM(playerid, COLOR_ADMIN, "Server sa reötartuje...");
	SendRconCommand("gmx");
	return 1;
}
...
{
    CreateDynamic3DTextLabel("[Banka]\nV›CHOD", 0xFF0000FF, 2305.8718,-16.1436,26.7496, 50.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 1, 0); // EXIT BANKA
    for(new bankPID=0; bankPID<MAX_PLAYERS; bankPID++)
	{
		CreateDynamicPickup(1274,1, 2305.8718,-16.1436,26.7496, bankPID, 0); // PICKUP EXIT
		CreateDynamicCP(2315.7192,-7.2789,26.7422, 1.5, 1, 0, bankPID); // V˝ber
	}
    ...
    CreateJob(JOB_NOVINAR,"Novin·r", -2054.9534,453.9141,35.1719,4);
    ...
    CreateJob(JOB_MAFINA,"Rusk· Mafia", X,61);
    ...
    */
    ...
    CreateJob(JOB_SKLADNIK,"SkladnÌk", -1933.8248,619.4207,35.1719,51);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(IsPlayerInSphere(playerid,-2026.7332,-114.9335,1035.1719,3)==1)
	{
			if(PRESSED(KEY_SECONDARY_ATTACK))
			{
				ShowPlayerDialog(playerid, DIALOG_CARSCHOOL, DIALOG_STYLE_LIST, "Autoökola", "Zkouöky (10 000$)", "Zvolit", "Zruöit");
			}
	}
	return 1;
}
		...
		if(dialogid == DIALOG_RADIO)
		{
		    if(response)
		    {
				switch(listitem)
				{
					...
					case 8: ShowPlayerDialog(playerid, DIALOG_OWNRADIO, DIALOG_STYLE_INPUT, "VlastnÌ r·dio", "Zde napiö odkaz", "Select", "Cancel");
				}
			}
		}
     	...
     	if(dialogid == DIALOG_LOGIN)
	    {
            if( response )
            {
				...
				if(udb_hash(inputtext) == DOF2_GetInt(UserPath(playerid), "Password"))
				{
					...
					{
					    new string[120];
					    format(string, sizeof(string), "Zadal si nespr·vne heslo %d/3 !", BadPass[playerid]);
						Message(playerid, string);
					}
					if(BadPass[playerid] >= 3)
					{
	...
	if(success)
 	{
        ...
        Message(playerid, "Tento prÌkaz neexistuje, pouûi prosÌm /help alebo /cmds");
	}
	return 1;
}
...
CMD:pm(playerid, params[])
{
	...
	if(sscanf(params, "ii",pid, message)) return SCM(playerid, -1, "ERROR: /pm <ID> <SPR¡VA>");
...
CMD:admins(playerid)
{
	...
	foreach(Player, i)
	{
 		if (PlayerInfo[playerid][Admin] >= 1)
   		{
     		format(string, sizeof(string), "Nick\tID\tAdminLevel\tPozÌcia\n\%s\t%d\t%d\t%s\n",PlayerName(i), i,GetALevel(i), GetNameALevel(i));
			count++;
   		}
	}
...
{
	ShowPlayerDialog(playerid, DIALOG_OPTIONS, DIALOG_STYLE_LIST, "Settings", "VypnutÌ TD\nPoloûka 2\nPoloûka 3", "Select", "Cancel");
	return 1;
}
...
CMD:atrestani(playerid)
{
	...
	strcat(string,"\n|{00CCFF}| Elek. öokov: {FFFFFF}%d{00CCFF} || Faciek: {FFFFFF}%d{00CCFF} || Samotka: {FFFFFF}%d{00CCFF} || VarovanÌ: {FFFFFF}%d{00CCFF} |");
	strcat(string,"\n| Vstupov na Event: {FFFFFF}%d{00CCFF} || ZodpovedanÈ dotazy: {FFFFFF}%d{00CCFF} || Admin V‰zenie: {FFFFFF}%d{00CCFF} || ZamrazenÌ: {FFFFFF}%d{00CCFF} |");
	strcat(string,"\n| VykopnutÌ: {FFFFFF}%d{00CCFF} || Admin Respawnov: {FFFFFF}%d{00CCFF} || Admin-ZabitÌ: {FFFFFF}%d{00CCFF} || ExploziÌ: {FFFFFF}%d{00CCFF}");
	strcat(string,"\n| Bany: {FFFFFF}%d{00CCFF} |");
...
stock GetNameALevel(playerid)
{
	...
	switch(GetALevel(playerid))
	{
		...
		case 4: ranka = "HlavnÌ Administrator";
		case 5: ranka = "VedenÌ";
		case 6: ranka = "VedenÌ + RCON";
	}
	return ranka;
}
...
CMD:setvip(playerid,params[])
{
    if(GetALevel(playerid) >= 1 || IsPlayerAdmin(playerid))
    {
        ...
        format(str, 128, "{FF0000}[ {FF0000}!{FFFFFF}] Administr·tor %s ti nastavil {FFFF00}VIP {FFFFFF} na level {FF0000}%d{FFFFFF}!", PlayerName(playerid), level);
        Message(pid, str);
        SaveData(pid);
        return 1;
    }else{
